...
For pharmaceutical and biologics manufacturers, the predicate-rule foundation sits in the US cGMP regulations: **21 CFR 211.68** (automatic, mechanical, and electronic equipment must be suitable and routinely checked), **21 CFR 211.100/211.110** (written procedures and in-process control of the process), and **21 CFR 211.22**, which makes the quality unit responsible for approving the procedures and specifications that affect product quality. Electronic records and signatures are governed by **21 CFR Part 11**. For process validation generally, the FDA's *Guidance for Industry: Process Validation: General Principles and Practices (January 2011)* establishes the three-stage lifecycle and the expectation that each stage produces documented conclusions.

For computerized systems specifically, the structural expectation comes through **EU GMP Annex 11 (Computerised Systems)**, whose principle requires documented evidence that a system is fit for its intended purpose and whose clause 4 expects validation documentation and reports to cover the relevant steps of the system life cycle. That is the clearest statement that a validation effort must end in a report, not just a stack of test records.

Where the system is part of a combination product (a drug or biologic combined with a device constituent), the device quality-system requirements also apply. **21 CFR 820.75** (now harmonized into the Quality Management System Regulation, QMSR, with a compliance date of 2 February 2026 that aligns it with ISO 13485:2016) requires process validation with documented results and approval, and requires review and revalidation when changes or deviations occur. Cite it as a lead basis only when the combination-product scope genuinely brings it into play; for a pure pharma or biologics system, the 211 series and Annex 11 carry the requirement.
