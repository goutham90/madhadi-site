...
For a combination product, the device constituent carries a parallel obligation in the control of nonconforming product. A failing test on a device part or its components is dispositioned through nonconformance and, where warranted, CAPA, the same logic that governs the drug-side OOS. The two streams have to stay aligned under the [pharmaceutical quality system](/articles/pharmaceutical-quality-system), with the same expectation of a documented, evidence-based decision rather than a comfortable one.
