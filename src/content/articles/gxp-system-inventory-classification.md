...
In the European Union, EU GMP Annex 11 is explicit on the point. Its general section calls for a current listing of all relevant systems together with their GMP functionality, which is the inventory itself, and it goes a step further for critical systems: those need a current system description that lays out the physical and logical arrangement, the data flows, the interfaces to other systems and processes, the hardware and software prerequisites, and the security measures in place.
...
|---|---|---|---|
| Category 1 | Infrastructure software | Operating systems, databases, middleware, virtualization, network OS | Qualified as infrastructure; relied upon by the layered apps; not validated as an application |
| Category 3 | Non-configured (used as supplied) | Firmware-driven instruments, off-the-shelf tools used with default settings, a balance with fixed output | Verify it meets requirements for the intended use; reuse supplier testing |
| Category 4 | Configured products | LIMS, MES, CDS, EDMS, ERP configured to your process via parameters and workflows, no code change | Validate the configuration and the workflows you built; risk-based functional testing of configured functions |
| Category 5 | Custom (bespoke) | In-house applications, custom code, complex spreadsheet macros, bespoke interfaces and scripts | Full lifecycle: requirements, design, code review, structural and functional testing; highest rigor |

There is no Category 2 in GAMP 5 second edition (it was retired with old "firmware" thinking). Two practical notes that trip people up:
