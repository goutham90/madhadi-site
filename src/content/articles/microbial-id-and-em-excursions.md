...
An **alert limit** is an established level that, when exceeded, gives early warning of a drift from normal conditions. Exceeding it is not necessarily a loss of control, but it requires attention and assessment. An **action limit**, when exceeded, requires investigation and corrective action because it signals that the process may have moved out of acceptable control. These definitions come from USP <1116> and are echoed in Annex 1 and the aseptic processing guidance.

A critical point that trips people up: in Grade A / ISO 5 critical zones, the expectation is essentially zero recovery. Annex 1 states that for Grade A the recommended action limit is "no growth," and any recovery of any organism is significant and investigated. You do not get to set a comfortable nonzero action limit for the critical zone. Lower grades (B, C, D) carry the limits laid out in Annex 1 tables and your own qualification data.
