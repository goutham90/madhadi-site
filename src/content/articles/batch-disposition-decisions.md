...
2. **Conditional/early release of finished product** before all data are final, which is generally **not** acceptable for commercial product going to patients. As a general rule, final release requires conformance to all final specifications per 211.165, though 211.165(a) itself carves out short-lived radiopharmaceuticals, and parametric or real-time release are recognized exceptions where registered.
...
For a product sterilized by a validated terminal moist-heat or other physical process, the sterility test on finished units is a weak control: it samples too few units to detect low-level contamination reliably and adds weeks of delay. Parametric release substitutes confirmation that the validated sterilization cycle ran within its proven physical parameters (for example, the achieved F0, time, temperature, and load configuration) plus the supporting bioburden and container-closure integrity data, in place of the end-product sterility test. Biological indicators support cycle validation and periodic re-qualification rather than serving as a routine per-batch release datum under a validated parametric-release scheme.
