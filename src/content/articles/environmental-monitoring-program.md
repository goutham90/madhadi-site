...
For viable count methods and media, **USP** chapters are the practical bench reference, `<1116>` (*Microbiological Control and Monitoring of Aseptic Processing Environments*) for program philosophy and the important framing of contamination recovery rates, `<797>` for compounding contexts, and `<61>`/`<62>` for microbial enumeration and absence of specified organisms on the nonsterile drug-product side (`<2021>`/`<2022>` are the nutritional and dietary-supplement counterparts to `<61>`/`<62>`, not the chapters to cite for a pharmaceutical product). `<1116>` is worth reading in full early; it is one of the few sources that talks candidly about what low recovery rates actually mean and why a single CFU in Grade A is not a statistical anomaly to be shrugged off.

> Annex 1 frames the whole exercise the same way: routine monitoring exists to show that the area is being held at a level of control suited to its required cleanliness, and the resulting data is expected to feed the contamination control strategy. EM is evidence for the CCS, not a standalone box to tick. See [the Annex 1 contamination control strategy](/articles/annex-1-contamination-control-strategy).
...
|-------|-------------|-----------------------------------|---------------------|------------------------------|-------------------------|
| A | Critical zone: open product, aseptic connections, filling | ~3,520 /m³ | No growth (target) | No growth (target) | No growth (target) |
...
| D | Support areas | at-rest limit applies; in-operation not predetermined (set in the CCS) | 200 CFU/m³ | 100 CFU | 50 CFU |

Two points the table cannot show but inspectors care about. First, the Grade A and B viable values are recovery *targets*; the expectation in Grade A is that you recover nothing, so any growth there is an event, not a tolerance band you operate inside. Second, the ≥5.0 µm macroparticle handling in Grade A changed in the 2022 Annex 1 revision (the ≥5.0 µm count was removed from Grade A/B classification and retained only as a routine-monitoring diagnostic), and the revision pushed continuous non-viable monitoring of Grade A and B during operations rather than periodic snapshots. If your program still relies on intermittent grab samples in the critical zone, that is a gap.
...
A frequent inspection finding: alert and action levels set at the regulatory maximum, so the program can never alert until the room is already at the legal limit. That defeats the purpose. If your Grade A action level is "any recovery breaches the limit," you have no alert headroom at all in the most critical zone, which is one reason the practical expectation in Grade A is to treat any recovery as an excursion regardless of a numeric "level."
