...
<div class="flow-v" role="img" aria-label="Use pattern sets the risk floor and the dominant control">
  <div class="flow-v-row"><div class="flow-box">Advisory / screening: model flags, a human decides</div><span class="flow-v-link" aria-hidden="true">floor: low &rarr;</span><div class="flow-box flow-box-test">Control: a real human review that samples what the model misses; watch false negatives.</div></div>
  <div class="flow-v-row"><div class="flow-box">Automated classification / action: output acted on without per-record review</div><span class="flow-v-link" aria-hidden="true">floor: medium &rarr;</span><div class="flow-box flow-box-test">Control: performance on the consequential class + monitoring from day one + defined escalation.</div></div>
  <div class="flow-v-base"><div class="flow-box flow-box-build">Process control / influence: model moves a physical parameter &rarr; floor: high. Control: an engineered, deterministic safety interlock independent of the model, plus model FMEA.</div></div>
</div>
...
- **EU GMP Annex 22, "Artificial Intelligence"** is the first EU GMP text written specifically for AI and machine learning in the manufacture of active substances and medicinal products. It was released in draft on 7 July 2025 (together with a revised Annex 11 and Chapter 4) for a consultation that closed on 7 October 2025, with a final version expected from 2026. In draft it emphasizes defining the model's intended use, setting performance metrics, controlling training and test data quality, and maintaining continuous oversight (change control, performance monitoring, and human review where needed), which lines up closely with the method in this article. It is a draft, not yet in force, so read it for direction and do not cite it as binding. See [the EU AI Act and life sciences](/articles/eu-ai-act-life-sciences) for how the GMP and horizontal-law tracks interact.
- **Guiding Principles of Good AI Practice in Drug Development** (published jointly by FDA and EMA on 14 January 2026) set out ten non-binding, principles-based statements covering AI across a medicine's lifecycle, from research and trials to manufacturing and safety monitoring, emphasizing human oversight, risk management, data governance, lifecycle control, and transparency. They are foundational principles meant to underpin future guidance rather than detailed requirements, so use them to frame the reasoning in an assessment, not as a checklist to certify against.
