---
title: "IEC 62304 and the SaMD Lifecycle for Pharma and Combination Products"
description: "How device software is engineered and controlled under IEC 62304 safety classes, SOUP management, and the IMDRF SaMD framework, framed for combination products, digital health, and software that feeds GxP records, and where this meets GAMP 5 computerized system validation."
pubDate: 2026-06-20
tags: ["IEC 62304", "SaMD", "combination products", "GAMP 5", "FDA", "software lifecycle", "predetermined change control", "digital health"]
pillar: "csv-csa"
tier: "Advanced"
---

A pharma quality professional walks into a software audit and reaches for GAMP 5. The same person, asked to sign off on the companion app that pairs with a connected inhaler or the algorithm that reads a clinical trial image, suddenly needs IEC 62304. Both standards look at the same artifact, code that influences a patient, but through different regulatory lenses, with different deliverables, different vocabularies, and different definitions of what "validated" even means. The gap matters more every year, because pharma, biologics, and cell and gene therapy companies increasingly ship software that is itself regulated as a device: a dosing companion app, an algorithm embedded in a combination product, a digital biomarker captured in a trial, a cloud service that produces a result feeding a GxP record. These are not spreadsheets with a validation summary.

This article walks through how that software gets built and controlled. It starts with the question of when software crosses into device territory, builds through IEC 62304's lifecycle and safety classification, covers Software of Unknown Provenance (SOUP), then moves into the Software as a Medical Device (SaMD) framework and the regulatory machinery now governing AI/ML models that change over time. Throughout, it ties this back to the GAMP 5 computerized system validation (CSV) world that pharma practitioners already know, and marks the places where the two genuinely meet, because for a drug, biologic, or combination-product organization that is where the work actually lands.

A note on scope before we start. The focus here is the pharma, biotech, biologics, cell and gene therapy, and combination-product world. The same engineering applies to a pharma company shipping a connected inhaler with a companion app, a biologics firm running a diagnostic SaMD in the cloud that informs a trial endpoint, and a combination-product team building the software inside a drug-delivery device. The vocabulary and the deliverables are the same; only the intended use and the risk change.

## When is software a medical device?

The threshold question is jurisdictional and definitional, not technical. In the United States, the definition of a device sits in Section 201(h) of the Federal Food, Drug, and Cosmetic Act, an enumerated list that includes any "instrument, apparatus, implement, machine, contrivance, implant, in vitro reagent, or other similar or related article" intended for diagnosis, cure, mitigation, treatment, or prevention of disease, or intended to affect structure or function of the body, that does not achieve its primary purpose through chemical action or metabolism. Software can satisfy this on its own. This is exactly the line a pharma or biologics organization has to watch: a companion app or an algorithm tied to a drug or biologic can land on the device side of that definition even though the product it supports is a drug.
...
In the EU, the Medical Device Regulation (Regulation (EU) 2017/745, the MDR) treats software as a device in its own right, and its Rule 11 classifies most diagnostic and therapeutic software into Class IIa or higher, which pulls in Notified Body oversight. This matters chiefly as a comparison point: a pharma or biologics organization selling a SaMD or a combination product in Europe needs to know its software can be regulated separately from the drug, and that the EU classification is independent of the US one.
...
A useful mental model: IEC 62304 is to device software what FDA's device quality system regulation is to the device overall. It is the software-specific expression of design controls. The framing has shifted, though, and an article published after the transition has to get this right. FDA finalized the Quality Management System Regulation (QMSR) on 31 January 2024 (published 2 February 2024), and it took effect on 2 February 2026, replacing the legacy 21 CFR Part 820 Quality System Regulation by incorporating ISO 13485:2016 by reference. Under the QMSR, the former standalone design-control provision 21 CFR 820.30 is reserved, and design controls are now governed by ISO 13485 clause 7.3, "design and development," incorporated by reference via 21 CFR 820.10. The substance carries over: the elements formerly enumerated under 820.30 (design inputs, design outputs, design review, design verification, design validation, design transfer, and the design history file) map onto the IEC 62304 deliverables and now live within the ISO 13485 clause 7.3 structure, using FDA's adopted ISO terminology. An auditor will expect the software lifecycle artifacts to live inside, or be referenced by, the design and development records (historically the Design History File). Our article on [user requirements and traceability](/articles/user-requirements-and-traceability) covers that wrapper in detail, and the [pharmaceutical quality system](/articles/pharmaceutical-quality-system) article covers the broader quality system it sits inside.
...
That last block, the ongoing anomaly and vulnerability review, is the operational heart of SOUP management today, and it merges with cybersecurity. The expected practice is to maintain a **Software Bill of Materials (SBOM)** enumerating every component and version, and to continuously monitor known-vulnerability databases (such as the U.S. National Vulnerability Database and the CVE feeds) for entries affecting those components. FDA's premarket cybersecurity guidance, "Cybersecurity in Medical Devices: Quality Management System Considerations and Content of Premarket Submissions" (final, September 2023), formalized the SBOM expectation, and Section 524B of the FD&C Act (added by the Consolidated Appropriations Act, 2023) gave FDA statutory authority over device cybersecurity, including an SBOM requirement for cyber devices. Our article on [device software cybersecurity for SaMD](/articles/device-software-cybersecurity-samd) covers the SBOM and vulnerability program in depth.
...
A common error is to treat IEC 62304 as the risk standard. It is not. The device risk standard is **ISO 14971**, "Medical devices, Application of risk management to medical devices" (current edition ISO 14971:2019). IEC 62304's risk process is a software-specific extension that feeds into, and draws from, the overarching ISO 14971 risk management process and its risk management file. For pharma readers, ISO 14971 is the device-side counterpart to the ICH Q9 quality risk management discipline; our [quality risk management](/articles/quality-risk-management) article covers that parent process, and this section covers the software seam.
...
**Validation is not the same word in each world.** This is the trap. In GAMP 5 and Part 11, "validation" means demonstrating fitness for intended use of a computerized system in its operating environment, owned by the user organization. In the device world, "software verification" and "design validation" are distinct regulatory terms with specific meanings under IEC 62304 and the design and development requirements (preserved but restructured under ISO 13485 clause 7.3, "design and development," now that the QMSR has reserved the former 21 CFR 820.30), owned by the manufacturer. When a pharma team and a device team both say "the software is validated," they may mean materially different things. Aligning that vocabulary early is the single most useful thing a cross-functional quality organization can do.
...
If you sit for a SaMD, combination-product, or digital health quality role in a pharma or biologics organization, expect these. Short, correct, confident answers follow.
...
**"How is IEC 62304 validation different from GAMP 5 validation?"** Different artifact and different owner. IEC 62304 plus design controls (now under ISO 13485 clause 7.3 design and development since the QMSR took effect on 2 February 2026, replacing the former 21 CFR 820.30) govern when the software *is* the device, owned by the manufacturer, with verification and design validation as defined terms. GAMP 5 and Part 11 govern when the software *supports a regulated process*, owned by the user organization, where "validation" means fitness for intended use in the operating environment.
...
For adjacent reading, see [user requirements and traceability](/articles/user-requirements-and-traceability), [quality risk management](/articles/quality-risk-management), [device software cybersecurity for SaMD](/articles/device-software-cybersecurity-samd), [the GAMP 5 CSV framework](/articles/gamp5-csv-framework), [IND, NDA, and BLA pathways](/articles/ind-nda-bla-pathways), and [GxP ML model lifecycle](/articles/gxp-ml-model-lifecycle).
