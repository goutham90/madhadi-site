...
> "AI systems referred to in Annex III shall be considered to be high-risk." - Regulation (EU) 2024/1689, Article 6(2)
...
**Route 1, Annex I (Article 6(1)): AI inside a regulated product.** If an AI system is a safety component of, or is itself, a product covered by listed EU product-safety law *and* that product already needs third-party conformity assessment, the AI is high-risk. The Medical Devices Regulation (EU) 2017/745 (MDR) and the In Vitro Diagnostic Regulation (EU) 2017/746 (IVDR) are on that list. So an AI-enabled medical device or IVD in a class that requires a notified body, broadly MDR Class IIa and above, or IVDR Class B and above, is high-risk under the AI Act by this route. This is the single most consequential overlap for device makers and is covered in detail below.
...
## The Medical-Device Overlap, in Detail

This is the part device and diagnostics companies cannot afford to misread, so work it step by step.

**WHY it matters.** An AI-enabled medical device or IVD now sits under *two* EU regimes at once: the sector law (MDR or IVDR) and the AI Act. They are not alternatives. The AI Act adds requirements; it does not displace MDR/IVDR. The risk is double-counting effort, or worse, assuming MDR compliance covers the AI Act and finding a gap at audit.
...
**WORKED EXAMPLE.** A company makes an AI-enabled IVD that flags likely-positive samples for analyst confirmation. Under IVDR it is Class C, so a notified body is already involved, which makes it high-risk under the AI Act. The team does not start over. It maps the IVDR risk file to the AI-Act risk-management requirement, extends the IVDR technical documentation with an AI annex (model description, intended purpose, data lineage, performance against a held-out test set, human-oversight design), adds explicit data-governance records showing the training data was representative across sites and sample types, and confirms the notified body will assess the AI-Act elements inside the IVDR procedure. The output is one extended file and one combined assessment, not two programmes. The deeper device-specific treatment is in [AI and machine learning in medical devices (SaMD)](/articles/ai-ml-medical-devices-samd) and the regulatory frame is in [EU MDR and IVDR overview](/articles/eu-mdr-ivdr-overview).
...
| Typical life-sciences example | A device maker building AI-enabled diagnostics; a company building its own quality model for internal use | A manufacturer or hospital buying and running a commercial AI tool |
...
**The change to flag, clearly, as not yet settled law.** Through 2025 the rollout fell behind, in large part because the harmonised standards and guidance companies need to implement the high-risk rules were not ready. In November 2025 the European Commission proposed a package, widely referred to as the Digital Omnibus on AI, that would defer the high-risk deadlines. As of mid-2026 the EU institutions had reached a *provisional political agreement* (reported around early-to-mid May 2026) under which the Annex III high-risk deadline would move to 2 December 2027 and the Annex I embedded-product deadline would move to 2 August 2028, among other simplifications.
...
5. **For high-risk device AI, run the MDR/IVDR overlap as one programme.** Map, extend, and combine the assessment as described above; do not build a parallel file.
...
**"How does it overlap with the medical-device rules?"** An AI-enabled device or IVD that already needs a notified body under MDR or IVDR is high-risk under the Act via the Annex I route. The two regimes are additive but designed to be assessed together: extend the existing technical documentation and run one combined conformity assessment, do not build a parallel file.
