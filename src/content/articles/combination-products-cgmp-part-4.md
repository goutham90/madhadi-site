...
description: "How drug-led and biologic-led combination products (prefilled syringes, autoinjectors, on-body delivery systems) are manufactured to cGMP: primary mode of action, the streamlined Part 4 approach, which provisions you add to a drug or biologics base system, and how to survive an inspection."
pubDate: 2026-06-21
tags: ["combination products", "21 CFR Part 4", "cGMP", "drug-device", "device constituent", "QMSR", "quality systems"]
pillar: "quality-assurance"
tier: "Intermediate"
---

A prefilled syringe is a drug and a device. So is an autoinjector, a pen injector, an on-body delivery system for a biologic, a transdermal patch, a metered-dose inhaler, and a kit that pairs a lyophilized vial with its diluent and a reconstitution set. Each combines constituents that, on their own, would sit under different regulations with different vocabularies, different inspectors, and different assumptions about what "in control" means. For a drug, biologic, or cell and gene therapy company, the delivery system is no longer an accessory bought in from a vendor, it is a regulated constituent of the product, and the manufacturing rules that govern the whole are a deliberate splice of the drug cGMP you already run and the device controls you may not.

This article covers what a combination product is, who decides which rules apply, the streamlined cGMP framework in 21 CFR Part 4, exactly which provisions you bolt onto a drug, biologics, or device base system, how to build and run a compliant operation, and the inspection findings that recur. The goal is that after reading it you can scope the quality system for a combination product, defend the design of that system to an inspector, and answer the questions an interviewer will ask.
...
- **Single-entity.** The constituents are physically or chemically combined into one product. A prefilled syringe, an autoinjector for a biologic, a transdermal patch, and an antibody conjugated to a small molecule are single-entity combinations.
...
- If the **drug or biologic** provides the PMOA, the product is drug-led or biologic-led. The base is the drug cGMP regulation, 21 CFR Parts 210 and 211. For a biologic constituent, 210 and 211 remain the cGMP base for the finished product (per 21 CFR 600.21 and the applicability of 211 to licensed biologics), with the biologics-specific standards in Parts 600 to 680 layered on top rather than replacing it.
...
A prefilled syringe is drug-led: the drug does the work, the syringe delivers it. An autoinjector or on-body delivery system for a biologic is drug-led for the same reason: the therapeutic protein provides the action, the device delivers it. The device-led case is the mirror image, where the device does the primary work and a drug coating plays a supporting role, such as a drug-eluting stent where the stent holds the vessel open and the drug suppresses restenosis. Every product gets sorted the same way, by asking what the most important therapeutic action is.
...
**1. Confirm PMOA and base.** The biologic provides the primary therapeutic action, so the product is biologic-led. For a therapeutic protein or monoclonal antibody, the lead center is CDER (which has reviewed therapeutic biologics since the June 2003 transfer; CBER retains vaccines, blood and blood products, allergenics, and cell, gene, and tissue products), and the product is filed as a BLA. The base operating system is 21 CFR 210 and 211 with the biologics standards in Parts 600 to 680 layered on. Document this determination and keep the rationale on file. If there was an RFD or Pre-RFD interaction, file that correspondence.
...
**3. Stand up design controls for the device constituent.** The autoinjector is a device constituent. It needs a design and development plan, design inputs derived from user needs and intended use, design outputs (drawings, specifications, software requirements if it is electromechanical), design reviews at defined gates, verification that outputs meet inputs, validation that the device meets user needs including a summative human factors study, design transfer to manufacturing, and a design history file. Use error is a primary risk for a self-administered injector, so human factors and use-related risk analysis under IEC 62366-1 are central. See [User Requirements and Traceability](/articles/user-requirements-and-traceability) and [Quality Risk Management](/articles/quality-risk-management).
...
For the device-constituent foundations, see [User Requirements and Traceability](/articles/user-requirements-and-traceability), [Quality Risk Management](/articles/quality-risk-management), and [Pharmaceutical Quality System](/articles/pharmaceutical-quality-system). For the drug-side foundations, see [21 CFR 210/211 cGMP Walkthrough](/articles/cfr-210-211-cgmp-walkthrough) and [Stability Programs and ICH Q1](/articles/stability-programs-ich).
