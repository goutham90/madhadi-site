...
### A second worked example, devices

The framework is not pharma-only. A medical device line records a nonconformance: a torque value on a critical fastener logged 0.9 N·m against a specification of 1.2 to 1.6 N·m. Immediate cause: the torque driver had drifted out of calibration. Scope: every unit assembled on that station since the last verified-good calibration check, 14 days prior, roughly 4,100 units. Root cause: the calibration interval was set on a fixed annual schedule with no in-use verification, so two weeks of drift went undetected. Disposition: units in inventory re-tested for fastener integrity; units already shipped assessed for [postmarket surveillance](/articles/device-postmarket-surveillance-mdr) and a possible field action. CAPA: add a daily station-level torque verification and shorten the calibration interval based on observed drift, fed through the [calibration and metrology program](/articles/calibration-and-metrology-program). Same logic, different modality.
