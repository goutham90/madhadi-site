...
The table below restates the published Annex 1 particle limits in plain terms; the values themselves are the regulatory thresholds, and the wording here is paraphrased rather than copied from the source. Limits are the maximum permitted >=0.5 um particles per cubic metre.

| EU GMP grade | At rest | In operation | Rough ISO 14644-1 equivalent | Typical use |
|---|---|---|---|---|
...
| D | 3,520,000 | Not predetermined for grade D in operation | ISO 8 at rest | Earlier stages, component prep |

Annex 1 also addresses the larger >=5 um particle size. For grade A and grade B at rest it classifies that size as "not specified" rather than carrying the old 2008 numeric value, and for routine monitoring it uses a recovery-based figure (29 particles/m3 in grade A) and treats a sustained count at that size as an early warning of a developing problem. The numbers in the table are the headline >=0.5 um limits, which is what most people quote.
...
The mechanics: a sterile, filterable, growth-promoting medium, usually **soybean-casein digest medium (tryptic soy broth, TSB)**, replaces the product. The line is set up and run exactly as for product. Units are filled, stoppered, and sealed. They are then **incubated** and read for growth. The standard incubation is a two-stage scheme, commonly **7 days at 20-25 C followed by 7 days at 30-35 C**, totaling 14 days, to recover fungi and molds (favored at 20-25 C) and bacteria (favored at 30-35 C) (the order of the two temperature stages can be reversed if justified). Before relying on any of this, the lab must confirm the medium is **growth-promoting** by inoculating a set of post-incubation units (or a parallel set) with a low challenge (<=100 CFU) of compendial test organisms plus relevant environmental isolates and showing growth within the incubation period; a media fill read as "no growth" is meaningless unless the medium has been proven able to support growth.

Now the number that matters. The **acceptance criterion for the modern standard is zero positive units** at the relevant batch size. The current Annex 1 (2022) expectation is unambiguous: the target is zero growth, and any single contaminated unit fails the simulation and triggers an investigation plus revalidation (typically three consecutive successful runs to re-qualify). The older tiered scheme below, where a single positive in a larger run could be handled through investigation alone, comes from the FDA's 2004 guidance and is the framing many U.S. programs grew up with; under the current Annex 1 it is superseded, but a quality unit should recognize the historical wording because it still appears in legacy procedures.

| Number of units filled | Older tiered handling (FDA 2004 guidance) |
|---|---|
| Fewer than 5,000 | No contaminated units expected. A single contaminated unit prompts an investigation and revalidation. |
| 5,000 to 10,000 | A single contaminated unit prompts an investigation, possibly a repeat run. A second contaminated unit prompts revalidation after the investigation. |
| More than 10,000 | A single contaminated unit prompts an investigation. A second contaminated unit prompts revalidation after the investigation. |

The practical reading every quality unit should carry: **the target is zero growth.** The tiered language above describes one historical way to *handle* a contaminated unit, not a license to expect one, and it no longer reflects the current Annex 1 position. Annex 1 states the goal plainly: the expectation is zero growth, and **any** contaminated unit is a failed simulation that must be investigated as a possible failure of the aseptic process. A single positive in a media fill is never a clerical event to be waved off; it is a contamination event on a line that is supposed to make sterile product, and it gets a full root-cause investigation: identify the organism (and compare it against the EM and personnel isolate library), review every intervention and operator on the run, examine the video if the line is monitored, and assess impact on product made on that line since the last successful media fill.
