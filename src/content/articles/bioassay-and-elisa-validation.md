...
| USP <1033> | Validation of Biological Assays | The validation chapter for bioassays. Defines relative accuracy, intermediate precision, range, and how to set acceptance criteria using the format of the reportable value. |
...
The modern approach in USP <1034> uses **equivalence testing**. The idea: pick a metric that captures how much the two curves differ in shape (for a 4PL, that is the slope and the two asymptotes, or a single combined similarity score), build a confidence interval around the sample-to-reference difference or ratio of that metric (90 percent is typical), and then ask whether that whole interval lands inside a set of pre-defined **equivalence bounds**. An interval contained within the bounds means the curves are close enough to call equivalent (parallel) and the run is valid. An interval that pokes past either bound means the sample is non-parallel.
...
Parallelism is the demonstration that sample and reference curves have the same shape and differ only by a horizontal shift, which is the assumption that makes the potency ratio meaningful at every response level. It should be tested by equivalence testing per USP <1034>: put a confidence interval around the curve-shape comparison and check that the interval stays inside pre-defined, data-derived equivalence bounds. The old difference-test approach (fail to find a significant difference equals parallel) is backwards because it rewards imprecise assays.
